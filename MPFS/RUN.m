...
path_noisy = 'E:\githubcode\refinenet\refinenet-master\refinenet-master\matlab\no_noise_xyz_normal';
path_result = '..\results\initial_normals';
shape_filenames = 'test_all.txt';
...
for i = 1:nfiles
    filename = files{i};
    V = read_xyz(fullfile(path_noisy, [filename, '.txt']));
    pts = V';
    pnums(i) = length(pts);
...
%     mean_errors(i) = mean(errors);
    ...
    write_xyz(fullfile(path_result, [filename, '.txt']), V, N);
end
